...
def pushData():
    cursor.execute("INSERT INTO det_bill(srno,particulars,qty,price,amount) VALUES (%s,%s,%s,%s,%s)",
                   (SrNo,particulars, quantity, price, amount))
    con.commit()

def pushHeader():
    cursor.execute("INSERT INTO det_bill(srno,particulars) VALUES (%s,%s)",
                   (SrNo," ".join(headerText)))
    con.commit()
